...
public:
	...
	void SetFrameTime(float frameTime_){ frameTime = frameTime_; }
	void Release();
	...
	std::map<int,std::vector<btVector3>> ReturnBulletVertex(MODEL_ID id, Matrix4& matWorld, CAMERA_ID cID);
	std::map<int, std::vector<int>> ReturnBulletIndex(MODEL_ID id);
	...
	void BindAnimation(ConstActorPtr actor);
	void SetAnimation(ConstActorPtr actor, ANIM_ID id, float animTime, float* updateAnimTime, ANIM_ID otherID, float otherAnimTime, float* otherUpdateAnimTime, float blendLevel, bool roop, float* beforeAnimBlendSpeed);
	void SetAnimation(ConstActorPtr actor, ANIM_ID id, float animTime, float* updateAnimTime, bool roop, float* beforeAnimBlendSpeed);
...
private:
	...
	std::map<ConstActorPtr, std::list<AnimParameter*>> animParameter;
	float frameTime;
};
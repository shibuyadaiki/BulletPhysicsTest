...
void Graphic::DrawMesh(MODEL_ID id, const Vector3& position, const Vector3& scale, const Vector3& rotate, CAMERA_ID cID, D3DXCOLOR* color, bool alphaFlag){
	Matrix4 mat =
		RCMatrix4::scale(scale) *
		RCMatrix4::rotateZ(rotate.z) *
		RCMatrix4::rotateX(rotate.x) *
		RCMatrix4::rotateY(rotate.y) *
...
std::map<int, std::vector<btVector3>> Graphic::ReturnBulletVertex(MODEL_ID id, Matrix4 & matWorld, CAMERA_ID cID)
{
	return mesh[id].ReturnBulletVertex(matWorld,cID);
}
std::map<int, std::vector<int>> Graphic::ReturnBulletIndex(MODEL_ID id)
{
...
void Graphic::BindAnimation(ConstActorPtr actor){
	...
	if (animParameter.size() > 0){
		...
		animation.BindAnim(&shader[sID], animMat);
	}

	animBind = true;
}
void Graphic::SetAnimation(ConstActorPtr actor, ANIM_ID id, float animTime, float* updateAnimTime, ANIM_ID otherID, float otherAnimTime, float* otherUpdateAnimTime, float blendLevel, bool roop, float* beforeAnimBlendSpeed){
...
void Graphic::SetAnimation(ConstActorPtr actor, ANIM_ID id, float animTime, float* updateAnimTime, bool roop, float* beforeAnimBlendSpeed){
	AnimParameter* ap = new AnimParameter();
	ap->animMat = id;
	ap->roopFlag = roop;
	ap->updateAnimTime = updateAnimTime;
...
void Graphic::DrawSphere(const Vector3 pos, const float radius, CAMERA_ID cID, const Vector3 color, const float alpha){
	...
	Graphic::GetInstance().SetTechniquePass(SHADER_ID::SPHERE_SHADER, "TShader", "P0");
	float size = radius;

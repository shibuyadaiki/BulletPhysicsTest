...
HRESULT Mesh::Load( const char* modelName){
	...
	for (int meshCountFor = 0; meshCountFor < meshCount + 1; meshCountFor++){
		...
		for (int i = 0; i < (*m.ReturnVertex())[meshCountFor].size(); i++) {
			bulletVertex[meshCountFor].push_back(RConvert(&(*m.ReturnVertex())[meshCountFor][i]));
		}
		...
		for (int indexSizeFor = 0; indexSizeFor < indexSize[meshCountFor]; indexSizeFor++){
			...
			bulletIndex[meshCountFor].push_back(idx);
			...
			vertexVec.push_back({ (*m.ReturnVertex())[meshCountFor][idx],
				...
				{ (int)((*m.ReturnWeight())[meshCountFor][idx][0] * 100), (int)((*m.ReturnWeight())[meshCountFor][idx][1] * 100), (int)((*m.ReturnWeight())[meshCountFor][idx][2] * 100, (*m.ReturnWeight())[meshCountFor][idx][3] * 100) }
			});
		}
...
std::map<int, std::vector<btVector3>> Mesh::ReturnBulletVertex(Matrix4& matWorld,CAMERA_ID cID)
{
	std::map<int, std::vector<btVector3>> vV;
	for (int j = 0; j < bulletVertex.size(); j++) {
		for (auto i : bulletVertex[j]) {
			Vector3 v = i;
			v = v * matWorld;
			vV[j].push_back(btVector3(v.x, v.y, v.z));
		}
	}
	return vV;
}

std::map<int, std::vector<int>> Mesh::ReturnBulletIndex()
{
	return bulletIndex;
}
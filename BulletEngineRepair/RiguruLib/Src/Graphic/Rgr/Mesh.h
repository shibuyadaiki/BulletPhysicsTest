...
public:
	...
	std::map<int,std::vector<btVector3>> ReturnBulletVertex(Matrix4& matWorld, CAMERA_ID cID);
	std::map<int,std::vector<int>> ReturnBulletIndex();
private:
	// 頂点定義
	struct VERTEX
	{
	...
	LoadTexture lt;
	RgrLoader rL;
	std::map<int, std::vector<Vector3>> bulletVertex;
	std::map<int, std::vector<int>> bulletIndex;
};